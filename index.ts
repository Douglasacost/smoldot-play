...
async function run() {
  ...
  const maxBatchSize = process.argv[3] ? parseInt(process.argv[3]) : 1;
  const batchInterval = process.argv[4] ? parseInt(process.argv[4]) : 1000;
  const client = new BatchJsonRpcClient({ testnet, maxBatchSize, batchInterval });
  await new Promise((resolve) => setTimeout(resolve, 4000));

  client.call('system_health', [], (response: string) => {
    console.log('response system_health:: ', response);
  });

  client.call('author_submitAndWatchExtrinsic', [hex], (response: string) => {
    console.log('response author_submitAndWatchExtrinsic:: ', response);
  });
}
